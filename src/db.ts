...
/**
 * Phase 4 (INFRA-06): read-only handle accessor for modules that need to run
 * prepared statements against the same DB (e.g. src/cost-ledger.ts).
 * Throws if called before initDatabase()/_initTestDatabase().
 */
...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    -- two concurrent INSERTs with the same PK both fail gracefully on the
    -- second attempt (SQLite serialises writes). UNIQUE idempotency_key
    -- prevents double-book from collisions. Both constraints are Pitfall-7
    -- (daily-cap DB race) mitigations. Schema mirror in src/cost-ledger.ts.
    CREATE TABLE IF NOT EXISTS voice_outbound_attempts (
      ...
      outcome             TEXT,              -- NULL | 'success' | 'no_answer' | 'busy' | 'voicemail' | 'escalated' | 'out_of_tolerance' | 'schedule_failed'
      idempotency_key     TEXT NOT NULL,     -- 64 hex; freshly minted per attempt by voice_outbound_schedule_retry
      originating_call_id TEXT,              -- audit-level, optional
      restaurant_name     TEXT,              -- legacy column, not authoritative
      created_at          TEXT NOT NULL,
  ...
  // Phase C migration (one-shot, idempotent): rename the legacy table on
  // existing prod DBs that still hold rows under the old name. SQLite
  // gracefully ignores the rename if the source table is absent (NEW DBs go
  // straight to voice_outbound_attempts via CREATE TABLE above). When the
  // legacy table exists alongside the new one, copy rows + drop old.
  try {
    const hasLegacy = database
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='table' AND name='voice_case_2_attempts'`,
      )
      .get();
    if (hasLegacy) {
      const newRows = database
        .prepare(`SELECT COUNT(*) as n FROM voice_outbound_attempts`)
        .get() as { n: number };
      if (newRows.n === 0) {
        // Empty new table → safe to ATTACH legacy via copy-then-drop.
        database.exec(
          `INSERT INTO voice_outbound_attempts SELECT * FROM voice_case_2_attempts;
           DROP TABLE voice_case_2_attempts;`,
        );
      } else {
        // Both populated (impossible in production but safe-guard anyway):
        // leave the legacy table; ops can manually consolidate.
      }
    }
  } catch {
    /* migration is best-effort; ignore if schema differs */
  }
