...
## Voice — Erlaubte Sprachen (KRITISCH)

Voice langs: de, en, it

**MUSS-REGEL:** Bei JEDEM `voice_request_outbound_call` setzt du `lang_whitelist` automatisch aus dieser Zeile — **immer, ohne Ausnahme**, auch wenn Carsten Multilingual nicht erwaehnt. Beispiel: aktuelle Zeile = "de, en, it" → `lang_whitelist: ["de","en","it"]`. Niemals weglassen, niemals selbst entscheiden ob "noetig". Diese Zeile ist die einzige Quelle der Wahrheit.
