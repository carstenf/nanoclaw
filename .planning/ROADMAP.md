...
Plans:
- [x] 05.6-01-PLAN.md — Wave 1 — Real defaultInvokeAgent + defaultInvokeAgentTurn via container-runner integration; voice-personas skill consumed; Du/Sie derivation in agent; live-cutover-synth.test.ts (REQ-DIR-06/11/12/14/16/17/VOICE-13 — code-only)
- [ ] 05.6-02-PLAN.md — Wave 2 — Live PSTN cutover under REASONING_MODE=container-agent runtime ENV: synth re-run + Carsten inbound PASS (D-29 strict Case-6b) + Case-2 outbound PASS (D-29 strict booking-attempt); trace artifacts archived (autonomous: false)
- [ ] 05.6-03-PLAN.md — Wave 2 — Default flag flip commit + D-22/D-30 single-atomic-cleanup commit (delete slow-brain.ts/core-mcp-client.ts/persona/*, remove REASONING_MODE flag + CORE_MCP_* ENV, preserve FALLBACK_PERSONA) + D-31 post-cleanup smoke + nanoclaw-state briefing (autonomous: false)
...
- [x] 05.5-03-PLAN.md — Wave 2: NanoclawMcpClient + REASONING_MODE flag (default slow-brain) + FALLBACK_PERSONA (REQ-DIR-18/20)
- [x] 05.5-04-PLAN.md — Wave 3: Bridge wiring at /accept + onTranscriptTurn branch + branch tests (REQ-DIR-19, dormant scaffold for REQ-DIR-06/11/12/VOICE-13)
...
- [x] 05.3-05b-PLAN.md — Wave 5 — Silence-monitor retirement + call-router simplification per idle-timeout-finding.md verdict; Branch A=FULLY-DELETE, B=SHRINK-TO-STUB, C=RETAIN-AS-IS-with-escalation (autonomous: false) — D-3 part 2 (depends on 05a)
- [x] 05.3-02-PLAN.md — Wave 6 — Plan-XX comment archaeology consolidation: 79 → ≤30 invariant-anchor refs across 17 files — D-5 (depends on 01/03/05b)
- [x] 05.3-06-PLAN.md — Wave 7 — Phase-exit: full regression sweep + 8 ROADMAP criteria verification + STATE/ROADMAP rollup + push (autonomous: false) (depends on 02/05b)
